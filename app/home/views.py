...
from app.models import User, Area, Scenic, Travels, Collect, Suggestion, Userlog, Rec
...
def index():
    ...
    if user_id:
        recobj = Rec.query.filter_by(user_id=user_id).all()
        scenic = []
        for r in recobj:
            scenic.append(Scenic.query.filter_by(id=r.scenic_id)[0])
...
def about():
    ...
    data = pd.read_csv('spider/data1.csv', on_bad_lines='skip').drop_duplicates()

    data1 = data.sort_values(by='月销量', ascending=False)
    people_max_val = []
    people_max_num = []
    test = 1
    for index, row in data1.iterrows():
        people_max_val.append(row['景区名称'])
        people_max_num.append(row['月销量'])
        test += 1
        if test > 10:
            break

    data2 = data.sort_values(by='价格', ascending=False)
    price_max_val = []
    price_max_num = []
    test = 1
    for index, row in data2.iterrows():
        price_max_val.append(row['景区名称'])
        price_max_num.append(row['价格'])
        test += 1
        if test > 10:
            break
    ...
    people_price = []
    for index, row in data1.iterrows():
        people_price.append([row['月销量'], row['价格']])

    citycounts = list(data['城市'].value_counts().index)
    cityres = []
    for item in citycounts:
        cityres.append([item, data[data['城市'] == item]['月销量'].mean()])

    return render_template('home/about.html', params=locals())
